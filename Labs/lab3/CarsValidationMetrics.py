...
def CalcMeasures(method, y_pred, y_true, df_f1=df_f1, df_precision=df_precision,
                 df_recall=df_recall):
    df_f1.loc[len(df_f1)] = [method] + \
        list(f1_score(y_true, y_pred, average=None))

    df_precision.loc[len(df_precision)] = [method] + \
        list(precision_score(y_pred, y_true, average=None))

    df_recall.loc[len(df_recall)] = [method] + \
        list(recall_score(y_pred, y_true, average=None))
...
method = 'sigmoid support vector machine'
clf = svm.SVC(kernel='sigmoid', C=50)
y_pred = cross_val_predict(clf, X, Y, cv=cv)
CalcMeasures(method, y_pred, Y)
...
print('df_precision output: ')
print(df_precision)
print('\n')

print('df_recall output: ')
print(df_recall)
print('\n')
